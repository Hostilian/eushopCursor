...
export function getCountryLegalFramework(iso2: string): CountryLegalFramework | null {
  ...
  if (!EU_ISO2.has(upperIso2)) return null;
  return {
    iso2: upperIso2,
    countryName: country.name,
    legalFamily: 'eu',
...
export function getAllCountryLegalFrameworks(): CountryLegalFramework[] {
  return COUNTRIES.map((country) => getCountryLegalFramework(country.iso2)).filter(
    (entry): entry is CountryLegalFramework => entry !== null,
  );
}
