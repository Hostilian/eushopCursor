...
export function CountryRail({ countries }: { countries: CountrySeed[] }) {
  return (
    <ul className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
      {countries.map((c, i) => {
        const palette = countryPalette[c.iso2] ?? { primary: '#3B2F22', accent: '#FAF7F2', ink: '#1A1612' };
        return (
          <motion.li
            key={c.iso2}
          ...
          >
            <Link
              ...
              className="group block overflow-hidden rounded-3xl border border-ink/10 transition-transform duration-300 hover:-translate-y-1"
            ...
            >
              <div className="relative h-44 p-5">
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  ...
                  <p className="mt-1 text-xs uppercase tracking-widest opacity-70">{c.region}</p>
                </div>
              </div>
