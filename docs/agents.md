...
| Lane | Edit | Read-only |
| --- | --- | --- |
| **A — Web + UI** | `apps/web`, `packages/ui`, `packages/i18n`, `packages/tokens`, `packages/catalog` | Everything else |
...
1. **Before** substantive edits, add `docs/claims/EUSHOP-<lane>-<nnn>.yaml` (see [`docs/claims/_template.yaml`](./claims/_template.yaml) and [`docs/claims/README.md`](./claims/README.md)). Filename must match `id`.
...
**Merge-safe parallel backlog:** [cursor-parallel-backlog.md](cursor-parallel-backlog.md) (task IDs, hotspots, backlog checkboxes).

## Agent handoff template (`docs/manifesto.md` §9)
