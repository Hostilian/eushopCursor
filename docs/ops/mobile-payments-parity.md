...
- The mobile trip screen does not embed Stripe Elements; when a client secret is present, the app can prompt the user to **complete the hold on the website** (see `EXPO_PUBLIC_SITE_URL` in [`eas.json`](../../apps/mobile/eas.json) and `/reservations`).
