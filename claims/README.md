...
1. **Before editing** non-trivial code, add `claims/EUSHOP-<lane>-<nnn>.yaml` (copy from [`_template.yaml`](./_template.yaml)). The **`id` must match the filename** (e.g. `EUSHOP-B-042` → `EUSHOP-B-042.yaml`).
...
- **A** — `apps/web`, `packages/ui-web`, `packages/i18n`, `packages/design-tokens`, `packages/catalog-data`
...
See [AGENTS.md](../AGENTS.md) and [docs/cursor-parallel-backlog.md](../docs/cursor-parallel-backlog.md).
