...
export const requestsRouter = router({
  feed: publicProcedure.input(requestsFeedInput).query(async ({ ctx, input }) => {
    try {
      const where = and(
        eq(requests.status, 'open' as const),
        input.countryIso2 ? eq(requests.countryIso2, input.countryIso2) : undefined,
      );
      return await ctx.db
        .select()
        .from(requests)
  ...
  near: publicProcedure.input(requestsNearInput).query(async ({ ctx, input }) => {
    try {
  ...
  byId: publicProcedure.input(requestByPublicIdInput).query(async ({ ctx, input }) => {
  ...
  matchesFor: protectedProcedure.input(requestIdFieldInput).query(async ({ ctx, input }) => {
